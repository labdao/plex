...
func processIOTask(ioEntry IO, index int, jobDir, ioJsonPath string, verbose, local bool, fileMutex *sync.Mutex) error {
	...
	err = copyInputFilesToDir(ioEntry.Inputs, inputsDirPath)
	if err != nil {
		updateIOWithError(ioJsonPath, index, err, fileMutex)
	...
	if local {
		...
		for key, input := range ioEntry.Inputs {
			cid, err := ipfs.AddFileHttp(ipfsNodeUrl, input.Address.FilePath)
			if err != nil {
				updateIOWithError(ioJsonPath, inputIndex, err, fileMutex)
				return fmt.Errorf("error adding inputs to IPFS: %w", err)
			}
			if verbose {
				fmt.Printf("Added inputs file to IPFS with CID: %s\n", cid)
			...
			// Update the JSON file with the new IPFS CID
			err = updateIOWithCid(ioJsonPath, inputIndex, key, cid, fileMutex)
			if err != nil {
				return fmt.Errorf("error updating JSON with IPFS CID: %w", err)
			}
		...
		bacalhauJob, err := bacalhau.CreateBacalhauJob(cids, toolConfig.DockerPull, cmd, memory, toolConfig.GpuBool, toolConfig.NetworkBool)
		if err != nil {
			updateIOWithError(ioJsonPath, index, err, fileMutex)
...
func copyInputFilesToDir(inputs map[string]FileInput, dirPath string) error {
	...
	for _, input := range inputs {
		srcPath := input.Address.FilePath
