...
class ErrorInferrer {
    ...
    _isSubtraceErrorPropagated(trace, callSubtraceStepIndex) {
        ...
        if (trace.error?.error === exceptions_1.ERROR.OUT_OF_GAS &&
            call.error?.error === exceptions_1.ERROR.OUT_OF_GAS) {
            return true;
        }
    ...
    _isContractCallRunOutOfGasError(trace, callStepIndex) {
        ...
        if (trace.error?.error !== exceptions_1.ERROR.REVERT) {
            return false;
        }
        const call = trace.steps[callStepIndex];
        if (call.error?.error !== exceptions_1.ERROR.OUT_OF_GAS) {
            return false;
        }
