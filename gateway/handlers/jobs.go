...
func UpdateJobHandler(db *gorm.DB) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var job models.Job
		// if result := db.Preload("Outputs").Preload("Inputs").Preload("Flow").First(&job, "bacalhau_job_id = ?", bacalhauJobID); result.Error != nil {
		if result := db.Preload("Outputs").Preload("Inputs").First(&job, "bacalhau_job_id = ?", bacalhauJobID); result.Error != nil {
			if errors.Is(result.Error, gorm.ErrRecordNotFound) {
				http.Error(w, "Job not found", http.StatusNotFound)
			} else {
			...
			for _, fileEntry := range outputFileEntries {
				...
				if result.Error != nil {
					if errors.Is(result.Error, gorm.ErrRecordNotFound) {
						...
						// experimentTagName := "experiment:" + job.Flow.Name
						experimentTagName := "experiment:" + flow.Name
						experimentTagType := "autogenerated"

						if err := addTag(db, experimentTagName, experimentTagType); err != nil {
							log.Printf("Error adding experiment tag: %v", err)
							continue
						}
						...
						dataFile = models.DataFile{
							...
							Tags:      []models.Tag{generatedTag, {Name: experimentTagName, Type: experimentTagType}},
							Timestamp: time.Now(),
						}
