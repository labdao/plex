...
const CardWithHref = ({ href, children }: { href: string | undefined; children: ReactNode }) =>
  href ? (
    <Link href={href}>
      <Card className="border border-border hover:border-ring">{children}</Card>
    </Link>
  ) : (
    <Card className="relative border cursor-not-allowed border-border ">
...
export default function TaskCard({ name, slug, available }: { name: string; slug: string; available: boolean }) {
  return (
    <CardWithHref key={slug} href={available ? `/tasks/${slug}` : undefined}>
      ...
      <Image src={`/images/task-${slug}.png`} className="w-full h-auto" sizes="100vw" width={220} height={140} alt={name} />
    </CardWithHref>
  );
}
