    ...
    def run(self, t, sequence, df):
        ...
        for scorer in scorer_list:
            ...
            if scorer=='ESM2' or scorer=='esm2':
                ...
                # def append_chain_b(pdb_file_path, len_binder):
                #     # Read the existing PDB file to find the last atom serial number and residue index
                #     last_atom_serial_number = 0
                #     last_residue_index = 0
                #     with open(pdb_file_path, 'r') as file:
                #         lines = file.readlines()
                #         for line in lines:
                #             if line.startswith("ATOM"):
                #                 last_atom_serial_number = int(line[6:11].strip())
                #                 last_residue_index = int(line[22:26].strip())

                #     # Write back the lines to the file, excluding any existing 'TER' or 'END' lines
                #     with open(pdb_file_path, 'w') as file:
                #         for line in lines:
                #             if not line.startswith(("TER", "END")):
                #                 file.write(line)

                #     # Open the PDB file in append mode to add chain B
                #     with open(pdb_file_path, 'a') as file:
                #         # Append the TER record for chain A
                #         file.write(f"TER   {last_atom_serial_number + 1:5d}      PHE A {last_residue_index:4d}                                                      \n")

                #         # Generate and append ATOM records for the unknown residues
                #         for i in range(1, len_binder + 1):
                #             atom_serial_number = last_atom_serial_number + i
                #             residue_index = last_residue_index + i
                #             atom_record = f"ATOM  {atom_serial_number:5d}  CA  UNK B {residue_index:4d}    \n"
                #             file.write(atom_record)

                #         # Append the TER record for chain B
                #         file.write(f"TER   {atom_serial_number + 1:5d}      UNK B {residue_index:4d}                                                      \n")
                ...
                if t==0: # run colabfold
                    ...
                    append_chain_b(pdb_file_path, len(binder_sequence))
