...
hyperfine --warmup 1 --min-runs 10 --show-output --export-csv /tmp/rfdiffusion-colabdesign_${BACALHAU_API_HOST}.csv -L size small,medium,large -n "rfdiffusion-colabdesign_{size}" "BACALHAU_API_HOST=$BACALHAU_API_HOST plex_main init -t tools/colabdesign/_colabdesign-dev.json -i '{\"protein\": [\"tools/colabdesign/6vja_stripped.pdb\"], \"config\": [\"tools/colabdesign/{size}-config.yaml\"]}' --scatteringMethod=dotProduct --autoRun=true -a test -a benchmarking --maxTime 1440"
